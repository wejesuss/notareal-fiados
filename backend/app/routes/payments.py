...
def list_payments_for_purchase(
    purchase_id: int, params: PaymentListQuerySchema = Depends()
):
...
def edit_payment(purchase_id: int, payment_id: int, data: PaymentUpdateSchema):
    """Edit allowed fields of a payment (amount_cents, method, description, payment_date)."""
    updated = update_payment(
        purchase_id, payment_id, data.model_dump(exclude_none=True)
    )
    return {"message": "Pagamento atualizado com sucesso.", "payment": updated}


@router.put("/{payment_id}/activate", response_model=PaymentWithMessageResponseSchema)
@handle_service_exceptions
def activate_payment_by_id(purchase_id: int, payment_id: int):
