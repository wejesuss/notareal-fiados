...
class PaymentBase(CamelModel):
    amount_cents: int | None = Field(None, example=3000)
...
class PaymentCreateSchema(PaymentBase):
    amount_cents: int = Field(..., example=6000)
...
# ===== RESPONSE =====
class PaymentResponseSchema(CamelModel, AmountMixin):
    id: int
    purchase_id: int
    amount_cents: int
...
class PaymentListQuerySchema(BaseModel):
    limit: int | None = Field(
        default=None, ge=1, description="Número máximo de pagamentos na listagem"
    )
    offset: int = Field(
        default=0,
        ge=0,
        description="Número de pagamentos para ignorar antes da listagem",
    )
